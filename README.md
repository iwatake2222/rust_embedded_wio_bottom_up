# 組込みRustをボトムアップでやる - レジスタ直叩き、PAC、HAL、BSP -
...
- 書籍「基礎から学ぶ 組込みRust」の第5章で説明されている内容をやってみた、という記事です
    - 組込みでRustを使う際は、基本的にはHALやBSPを使うことが多いと思います。書籍でも、BSPを用いてWioの各デバイスを制御する方法の説明が大部分です
    - しかし、これだと実際に何がどうなっているのかの理解が自分は十分にできなかったため、あえて基礎的なところから順に抽象度を上げていこうと思います
    - 書籍でもこの内容は説明されていますが、コードが抜粋だけであったり、説明量が少なかったので自分でやってみました
...
- そのため、マイコン個別の設定(メモリレイアウト)が別途必要です
- `memory.x` というファイルにメモリレイアウトを記載して、cargoコマンドを実行するのと同じ階層(Cargo.tomlと同じ階層)に配置します
- 通常はマイコンのデータシートに記載のメモリマップを元に作成します。が、今回は下記に既に正解があるのでこれを使いました
...
- コードにボード(Wio)依存の情報がある
    - 制御したいのはUSER_LEDですが、USER_LED=PA15という情報がコード内に入っていますし、開発者がケアする必要があります
...
- atsamd51p (PAC)
    ...
    - 注意点として、このクレートが提供するのは、「マイコンのペリフェラルへのアクセス」です。CPUコア(Cortex-M)の機能 (SysTickや割り込み、FPUなど)は管轄外です
...
# HALを用いてペリフェラルを制御する
次に、HAL (Hardware Abstraction Layer) を用いてみます。HALがデバイス依存の制御を吸収し、一般的なAPIを提供してくれます。これによってだいぶコードがすっきりします。また、アプリケーション側のコードからunsafeを取り除くことができます。
...
- embedded-hal (HAlのトレイト)
    ...
    - このクレートはトレイトであり、別途ターゲットマイコンごとにHALを実装したクレートが必要になります
...
## 開発者が知っておく必要がある情報
- なし
    - せいぜい、Button1がプルアップされているので、押されていないとHigh、押されたらLowというくらい
...
- 低レベル、とはいえSysTickを用いたdelayモジュールは提供してくれています
- delayを正しく動かすために、AHBクロックの値を設定する必要があります。120MHzだと思ったらうまく動かず、少し変えた値を設定しました。
    - 多分、クロックジェネレータの設定をしていないためだと思われます(後述)
...
- BSP経由でプロセッサ固有の機能にもアクセスできます。が、GPIO(HAL)と同様に、プロセッサアクセスをre-exportしているだけなのでdelayに関する部分は先ほどのコードとほとんど同じになります
